...
func ParseFunction(path string) ([]*Function, error) {
	...
	for _, fn := range funcDecls {
		...
		if fn.Recv != nil {
			...
			err := printer.Fprint(&buf, fset, fn.Recv.List[0].Type)
			if err != nil {
		...
		if fn.Type.Params != nil {
			for _, i := range fn.Type.Params.List {
				var buf bytes.Buffer
				err := printer.Fprint(&buf, fset, i.Type)
				if err != nil {
					return nil, errors.Wrap(err, "printer.Fprint failed")
				}
...
func GenerateWrapper(goPath string, pkgPath string, pkg string, cls string) (string, error) {
	functions, err := ParseFunction(path.Join(goPath, pkgPath))
	if err != nil {
	...
	for _, function := range functions {
		...
		if function.Recv.Type != cls && function.Recv.Type != fmt.Sprintf("*%s", cls) {
			continue
		}

		buf.WriteString("\n")
		buf.WriteString(generateWrapperFunctionDeclare(function))
		buf.WriteString(" {")
...
func generateWrapperFunctionDeclare(function *Function) string {
	var buf bytes.Buffer

	buf.WriteString("func ")
	if function.Recv != nil {
		buf.WriteString(fmt.Sprintf("(%s %sWrapper)", function.Recv.Name, function.Recv.Type))
	}
